...
namespace {
  ...
  ExtMove* generate_castling(const Position& pos, ExtMove* moveList, Color us) {
    ...
    const Square K = KingSide ? WEST : EAST;
  ...
  ExtMove* generate_pawn_moves(const Position& pos, ExtMove* moveList, Bitboard target) {
    ...
    if (Type == CAPTURES || Type == EVASIONS || Type == NON_EVASIONS)
    {
        ...
        {
            assert(rank_of(pos.epSquare) == relative_rank(Us, RANK_6));
            ...
            b1 = pawnsNotOn7 & pos.attacks_from<PAWN>(pos.epSquare, Them);

            assert(b1);

            while (b1)
                *moveList++ = make<ENPASSANT>(pop_lsb(&b1), pos.epSquare);
        }
    }
...
ExtMove* generate<LEGAL>(const Position& pos, ExtMove* moveList) {
  ...
  Bitboard pinned = pos.pinned_pieces(pos.sideToMove);
  Square ksq = pos.square<KING>(pos.sideToMove);
